...
│    ├── CheckThread: Slackチャンネルで既存スレッド検索 │
...
→ CheckThread(Slack検索でスレッド判定) → HasThread(IF分岐)
...
# Slack Bot Token（再レビュー時の既存スレッド検索に使用）
# n8n の Credentials → Slack の Access Token と同じもの
...
5. **「Bot Token Scopes」** セクションで以下を追加:
   ...
   - `channels:history` — チャンネルの既存スレッド検索（再レビュー時）
...
| CheckThread → HasThread | Slackチャンネルから既存スレッドを検索し、正しく分岐しているか（true → ReReviewMessages, false → SlackParent） |
...
| 再レビューがスレッドに追記されない | `.env` の `SLACK_BOT_TOKEN` が設定されているか確認。HasThread (IF ノード) の条件が `{{ $json.hasThread }}` equals `true` (Boolean) になっているか確認。接続: true → ReReviewMessages, false → SlackParent |
| 再レビューが常に新規スレッドになる | `SLACK_BOT_TOKEN` 未設定、または Bot に `channels:history` スコープがない。Slack App の OAuth & Permissions で確認 |
...
> スレッド情報は Slack チャンネルの直近50件から検索するため、再インポート後も既存スレッドへの追記は継続される。
