...
> スレッド情報はファイル（`/home/node/.n8n/claude-review-threads.json`）にも永続化されるため、再インポート後も既存スレッドへの追記は継続される（2週間以内の投稿があるスレッドのみ）。
...
> スレッド情報はファイルに永続化されるため、再インポート後も既存スレッドへの追記は継続される。
